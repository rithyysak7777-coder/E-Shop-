...
const Products = () => {
  ...
  const getProductCountForCategory = (catId) => {
    const count = products.filter(
      (p) => Number(p.category_id) === Number(catId),
    ).length;
  ...
  const fetchStoreData = useCallback(async () => {
    try {
      ...
      console.log("✅ Extracted Products:", prodList);
      console.log("✅ Extracted Categories:", catList);
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") {
      ...
      console.log(
        `  ✅ Filtered by category ${selectedCategory}: ${result.length} products found`,
      );
    } else {
    ...
    if (searchQuery.trim() !== "") {
      ...
      console.log(
        `  🔍 Search for "${searchQuery}": ${result.length} products found`,
      );
    }
  ...
  return (
    <div className="container py-5">
      ...
      {isLoading ? (
        <LoadingSpinner message="Fetching products catalogue..." />
                      ...
                      >
                        {(() => {
                          const imgs =
                            ...
                            [];
                          const src =
                            imgs[quickViewActiveImage]?.image_url ||
                        ...
                        <div className="d-flex gap-2 mt-2 justify-content-center">
                          {(
                            ...
                            <button
                              key={img.id || idx}
                              type="button"
                              className={`p-0 border rounded-2 ${quickViewActiveImage === idx ? "border-primary" : "border-light"}`}
                              style={{
                                width: 64,
                                height: 64,
                                overflow: "hidden",
